...
import (
	...
	"user-service/internal/middlewares/token"
...
const JwtTokenBlackListSetKey = `user-service:jwt:token:blacklist`
...
func (s *UserService) Info(c *gin.Context) (user *models.User, err error) {
	user, err = repos.User.One(c, c.GetUint(`uid`))
	user.Passwd = ``
	user.Salt = ``
	user.Roles = s.UserRolesByUid(c, user.GetID())
	return
}
