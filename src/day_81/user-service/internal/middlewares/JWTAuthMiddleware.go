...
import (
	...
	token "user-service/internal/middlewares/token"
...
func JWTAuthMiddleware() func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		if err != nil {
			response.Error(c, "invaled Token", http.StatusUnauthorized)
			c.Abort()
			return
		}
		//根据admin_id查询用户是否存在
		...
		if (*user).GetID() <= 0 {
			response.Error(c, `user dose not exist`, http.StatusUnauthorized)
			c.Abort()
			return
		}
		// 判断token是否即将过期
